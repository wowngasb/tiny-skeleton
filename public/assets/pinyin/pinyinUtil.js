...
;(function (global, factory) {
    ...
    var getChinese = function (strValue) {
        if (strValue != null && strValue != "") {
            var reg = /[\u4e00-\u9fa5]/g;
            var ret = strValue.match(reg);
    ...
    var dict = {}; // 存储所有字典数据
    var cache_first_letter = {};
    var cache_pinyin_all = {};
    var cache_pinyin = {};

    var pinyinUtil =
        {
            _cache_first_letter: cache_first_letter,
            _cache_pinyin_all: cache_pinyin_all,
            _cache_pinyin: cache_pinyin,
            debug: false,
            /**
            ...
            pinyinStr: function (chinese, seq) {
                // pinyinUtil.pinyinStr("红叶直播(1249-叶总)");
                var key = chinese + '^' + (seq ? seq : '_');
                if (!cache_pinyin[key]) {
                    var ret = pinyinUtil._pinyinStr(chinese, seq);
                    cache_pinyin[key] = ret;
                }
                return cache_pinyin[key];
            },
            _pinyinStr: function (chinese, seq) {
                chinese = chinese || '';
                seq = seq || '-';
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    if (/^[a-zA-Z0-9]$/g.test(tmp)) {
                        ret = ret ? (ret + '-' + tmp) : tmp;
                    } else {
                        var tmpChar = removeChinese(tmp);
                        var tmpChinese = getChinese(tmp);

                        var l_list = tmpChinese ? pinyinUtil.getFirstLetter(tmpChinese, true) : [];
                        var l_list_2 = [];
                        for (var j = 0; j < l_list.length; j++) {
                            l_list_2.push(tmpChar + l_list[j]);
                        }
                        var ret_l = l_list_2.join('|').toLowerCase();

                        var p_list = tmpChinese ? pinyinUtil.getPinyin(tmpChinese, ' ', false, true).replaceAll(' ', '') : '';
                        var ret_p = p_list ? tmpChar + p_list : '';

                        if (ret_p && ret_l) {
                            ret = ret ? (ret + '-' + ret_p + '#' + ret_l) : ret_p + '#' + ret_l;
                        } else {
                            if (ret_p) {
                                ret = ret ? (ret + '-' + ret_p) : ret_p;
                            }
                            if (ret_l) {
                                ret = ret ? (ret + '-' + ret_l) : ret_l;
                            }
                            if (!ret_l && !ret_p && tmpChar) {
                                ret = ret ? (ret + '-' + tmpChar) : tmpChar;
                            }
                        }
                    }
                }
            ...
            getPinyin: function (chinese, splitter, withtone, polyphone) {
                var key = chinese + '^' + (splitter ? 'splitter' : 'none') + '^' + (withtone ? 'withtone' : 'none') + '^' + (polyphone ? 'polyphone' : 'none');
                if (!cache_pinyin_all[key]) {
                    var ret = pinyinUtil._getPinyin(chinese, splitter, withtone, polyphone);
                    cache_pinyin_all[key] = ret;
                }
                return cache_pinyin_all[key];
            },
            _getPinyin: function (chinese, splitter, withtone, polyphone) {
                ...
                {
                    if (withtone) pinyinUtil.debug && console.warn('pinyin_dict_notone 字典文件不支持声调！');
                    if (polyphone) pinyinUtil.debug && console.warn('pinyin_dict_notone 字典文件不支持多音字！');
                    var noChinese = '';
                    for (var i = 0, len = chinese.length; i < len; i++) {
             ...
             */
            getFirstLetter: function (chinese, polyphone) {
                var key = chinese + '^' + (polyphone ? 'polyphone' : 'none');
                if (!cache_first_letter[key]) {
                    var ret = pinyinUtil._getFirstLetter(chinese, polyphone);
                    cache_first_letter[key] = ret;
                }
                return cache_first_letter[key];
            },
            _getFirstLetter: function (str, polyphone) {
                polyphone = polyphone == undefined ? false : polyphone;
