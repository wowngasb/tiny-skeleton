...
def _render_column(column, show_name, doc = None):
    kwarg = []
...
class ModelClass(Model):
    ...
    def render(self, db_comments_map=None):
        ...
        _doc = lambda t, c , m: m[t][c] if m and c in m.get(t, {}) else ''
        # Render columns
        text += '\n'
        for attr, column in self.attributes.items():
            if isinstance(column, Column):
                show_name = attr != column.name
                doc = _doc(self.table.name, column.name, db_comments_map)
                text += '    {0} = {1}'.format(attr, _render_column(column, show_name, doc))
                text += '\n'
...
class CodeGenerator(object):
    ...
    def render(self, outfile=sys.stdout, db_comments_map=None):
